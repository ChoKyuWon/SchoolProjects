...
void _insert(BTreeNode* present, int k)
{
	...
	if (present->leaf == true)
	{
		...
		for(index = i; index > 0; index--){
			if(present->keys[index - 1] < k)
				break;
			present->keys[index] = present->keys[index - 1];
		}
		*/
		present->keys[i] = k;
		present->n++;
		_sort(present->keys, i+1);
		_balancing(present);
		return;
	...
	else // If this node is not leaf
	{
		...
		for(index=0;index<i;index++){
			if(present->keys[index] > k)
				break;
		}
		_insert(present->C[index], k);
		return;
...
void _remove(BTreeNode* present, int k)
{
	...
	int index;
	if(present->keys[0] > k)
		_remove(present->C[0], k);
	else if(present->keys[(present->n) - 1] < k){
		_remove(present->C[(present->n)], k);
	}
	else{
		for(index = 0; index < present->n; index++){
			if(present->keys[index] < k && present->keys[index + 1] > k){
				_remove(present->C[index + 1], k);
			}
			if(present->keys[index] == k){
				if(present->leaf == true){
					present->n--;
					if(present->n != 0){
						for(int i =index; i< present->n; i++)
					...
					int tmp = present->keys[index];
					present->keys[index] = present->C[0]->keys[(present->C[0]->n) - 1];
					present->C[0]->keys[(present->C[0]->n) - 1] = tmp;
					_sort(present->keys, present->n);
					_remove(present->C[0], k);
				}
			}
		}
